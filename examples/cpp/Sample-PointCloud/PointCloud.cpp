...
int main(int argc, char **argv) try {
    ...
    for (int i = 0; i < 30; ++i) {
      auto frameset = pipeline.waitForFrames(1000);  // 超时增加到 1 秒
      if (frameset) {
        std::cout << "Got frameset, depth="
                  << (frameset->depthFrame() ? "yes" : "no")
                  << ", color=" << (frameset->colorFrame() ? "yes" : "no")
                  << std::endl;
        if (frameset->depthFrame() && frameset->colorFrame()) break;
      } else {
        std::cout << "waitForFrames timeout" << std::endl;
      }
    }
